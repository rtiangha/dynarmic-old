...
void genVsib(bool isJIT) {
    ...
    const char* vm32xTbl[] = {
        "xmm0",         "xmm0 * 1",       "xmm0 + 4",
        "xmm0 + eax",   "xmm0 * 4 + ecx", "xmm3 * 8 + edi + 123",
        "xmm2 * 2 + 5", "eax + xmm0",     "esp + xmm2",
    };
    const char* vm32yTbl[] = {
        "ymm0",         "ymm0 * 1",       "ymm0 + 4",
        "ymm0 + eax",   "ymm0 * 4 + ecx", "ymm3 * 8 + edi + 123",
        "ymm2 * 2 + 5", "eax + ymm0",     "esp + ymm2",
    };
...
void genAddress(bool isJIT, const char regTbl[][5], size_t regTblNum) {
    ...
    for (size_t i = 0; i < regTblNum + 1; i++) {
        ...
        for (size_t j = 0; j < regTblNum + 1; j++) {
            if (j == 4)
                continue; /* esp is not index register */
