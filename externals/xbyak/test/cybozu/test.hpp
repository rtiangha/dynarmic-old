...
public:
    AutoRun() : init_(0), term_(0), okCount_(0), ngCount_(0), exceptionCount_(0) {}
    ...
    int run(int, char* argv[]) {
        ...
        try {
            ...
            for (UnitTestList::const_iterator i = list_.begin(), ie = list_.end(); i != ie; ++i) {
                ...
                try {
                    ...
                    exceptionCount_++;
                    std::cout << "ctest:  " << i->first << " is stopped by exception " << e.what()
                              << std::endl;
                } catch (...) {
                    exceptionCount_++;
                    std::cout << "ctest:  " << i->first << " is stopped by unknown exception"
                              << std::endl;
                }
            }
            if (term_)
        ...
        if (msg.empty()) {
            ...
            std::cout << "ctest:name=" << getBaseName(*argv) << ", module=" << list_.size()
                      << ", total=" << total << ", ok=" << okCount_ << ", ng=" << ngCount_
...
inline void test(bool ret, const std::string& msg, const std::string& param, const char* file,
                 int line) {
    autoRun.set(ret);
    if (!ret) {
...
#define CYBOZU_TEST_EQUAL_POINTER(x, y)                                                            \
    {                                                                                              \
        bool _cybozu_eq = x == y;                                                                  \
        cybozu::test::test(_cybozu_eq, "CYBOZU_TEST_EQUAL_POINTER", #x ", " #y, __FILE__,          \
                           __LINE__);                                                              \
        if (!_cybozu_eq) {                                                                         \
...
#define CYBOZU_TEST_EQUAL_ARRAY(x, y, n)                                                           \
    {                                                                                              \
        for (size_t _cybozu_test_i = 0, _cybozu_ie = (size_t)(n); _cybozu_test_i < _cybozu_ie;     \
             _cybozu_test_i++) {                                                                   \
            bool _cybozu_eq = cybozu::test::isEqual((x)[_cybozu_test_i], (y)[_cybozu_test_i]);     \
            cybozu::test::test(_cybozu_eq, "CYBOZU_TEST_EQUAL_ARRAY", #x ", " #y ", " #n,          \
                               __FILE__, __LINE__);                                                \
            if (!_cybozu_eq) {                                                                     \
...
#define CYBOZU_TEST_EXCEPTION_MESSAGE(statement, Exception, msg)                                   \
    {                                                                                              \
        ...
        if (_cybozu_ret) {                                                                         \
            cybozu::test::test(false, "CYBOZU_TEST_EXCEPTION_MESSAGE",                             \
                               #statement ", " #Exception ", " #msg, __FILE__, __LINE__);          \
            if (_cybozu_ret == 1) {                                                                \
...
#define CYBOZU_TEST_EXCEPTION(statement, Exception)                                                \
    {                                                                                              \
        ...
        if (_cybozu_ret) {                                                                         \
            cybozu::test::test(false, "CYBOZU_TEST_EXCEPTION", #statement ", " #Exception,         \
                               __FILE__, __LINE__);                                                \
            if (_cybozu_ret == 1) {                                                                \
                std::cout << "ctest:  no exception" << std::endl;                                  \
            } else {                                                                               \
