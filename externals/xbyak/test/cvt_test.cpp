...
const struct Ptn {
    ...
    {&al, ax, eax, rax},      {&bl, bx, ebx, rbx},      {&cl, cx, ecx, rcx},
    {&dl, dx, edx, rdx},      {&sil, si, esi, rsi},     {&dil, di, edi, rdi},
    {&bpl, bp, ebp, rbp},     {&spl, sp, esp, rsp},     {&r8b, r8w, r8d, r8},
    {&r9b, r9w, r9d, r9},     {&r10b, r10w, r10d, r10}, {&r11b, r11w, r11d, r11},
    {&r12b, r12w, r12d, r12}, {&r13b, r13w, r13d, r13}, {&r14b, r14w, r14d, r14},
    {&r15b, r15w, r15d, r15},
};
#else
const struct Ptn {
    ...
    {0, si, esi},   {0, di, edi},   {0, bp, ebp},   {0, sp, esp},
};
#endif
...
#ifdef XBYAK32
    {
        const Reg16 errTbl[] = {si, di, bp, sp};
        for (size_t i = 0; i < sizeof(errTbl) / sizeof(errTbl[0]); i++) {
...
CYBOZU_TEST_AUTO(changeBit) {
    ...
    const size_t N = 6;
    const Reg* tbl[][N] = {
        {&al, &ax, &eax, &xmm0, &ymm0, &zmm0}, {&cl, &cx, &ecx, &xmm1, &ymm1, &zmm1},
        {&dl, &dx, &edx, &xmm2, &ymm2, &zmm2}, {&bl, &bx, &ebx, &xmm3, &ymm3, &zmm3},
        {0, &sp, &esp, &xmm4, &ymm4, &zmm4},   {0, &bp, &ebp, &xmm5, &ymm5, &zmm5},
        {0, &si, &esi, &xmm6, &ymm6, &zmm6},   {0, &di, &edi, &xmm7, &ymm7, &zmm7},
    };
        ...
        for (size_t j = 0; j < N; j++) {
            ...
            for (size_t k = 0; k < N; k++) {
                if (tbl[i][k]) {
                    ...
                    // printf("%s->changeBit(%d)=%s %s\n", r1->toString(), bitTbl[k],
                    // r1->changeBit(bitTbl[k]).toString(), tbl[i][k]->toString());
                } else {
