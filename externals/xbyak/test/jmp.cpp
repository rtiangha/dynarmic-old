...
CYBOZU_TEST_AUTO(test1) {
    ...
    static const struct Tbl {
        ...
        {0, true, true, {0xeb, 0xfe}, 2},    {1, true, true, {0xeb, 0xfd}, 2},
        {126, true, true, {0xeb, 0x80}, 2},  {127, true, false, {0xe9, 0x7c, 0xff, 0xff, 0xff}, 5},
        {0, false, true, {0xeb, 0x00}, 2},   {1, false, true, {0xeb, 0x01}, 2},
        {127, false, true, {0xeb, 0x7f}, 2}, {128, false, false, {0xe9, 0x80, 0x00, 0x00, 0x00}, 5},
    };
...
CYBOZU_TEST_AUTO(testJmpCx) {
    struct TestJmpCx : public CodeGenerator {
        explicit TestJmpCx(void* p, bool useNewLabel) : Xbyak::CodeGenerator(16, p) {
            if (useNewLabel) {
                Label lp;
                L(lp);
#ifdef XBYAK64
...
CYBOZU_TEST_AUTO(test2) {
    struct TestJmp2 : public CodeGenerator {
        /*
         ...
         22                                  f4:
        */
        TestJmp2(void* p, bool useNewLabel) : Xbyak::CodeGenerator(8192, p) {
            if (useNewLabel) {
                inLocalLabel();
                nop();
                nop();
...
CYBOZU_TEST_AUTO(test3) {
    struct Grow : Xbyak::CodeGenerator {
        Grow(int dummySize) : Xbyak::CodeGenerator(128, Xbyak::AutoGrow) {
            mov(eax, 100);
            push(eax);
            call((void*)add5);
            add(esp, 4);
...
CYBOZU_TEST_AUTO(test4) {
    struct Test4 : Xbyak::CodeGenerator {
        Test4(int size, void* mode, bool useNewLabel) : CodeGenerator(size, mode) {
            if (useNewLabel) {
                Label x;
                jmp(x);
                putNop(this, 10);
...
CYBOZU_TEST_AUTO(test5) {
    struct Test5 : Xbyak::CodeGenerator {
        explicit Test5(int size, int count, void* mode) : CodeGenerator(size, mode, &myAlloc) {
            using namespace Xbyak;
            inLocalLabel();
            mov(ecx, count);
            xor_(eax, eax);
...
CYBOZU_TEST_AUTO(testF_B) {
    ...
    const int expectedTbl[] = {2, 0, 3, 3, 5, 2, 2, 6};
...
CYBOZU_TEST_AUTO(test6) {
    struct TestLocal : public Xbyak::CodeGenerator {
        TestLocal(bool grow) : Xbyak::CodeGenerator(grow ? 128 : 4096, grow ? Xbyak::AutoGrow : 0) {
            xor_(eax, eax);
            inLocalLabel();
            jmp("start0", T_NEAR);
            L(".back");
...
CYBOZU_TEST_AUTO(test_jcc) {
    ...
    struct B : Xbyak::CodeGenerator {
        B(bool grow, const void* p)
            : Xbyak::CodeGenerator(grow ? 0 : 4096, grow ? Xbyak::AutoGrow : 0) {
            mov(eax, 1);
            add(eax, 2);
            jnz(p);
        }
...
CYBOZU_TEST_AUTO(testNewLabel) {
    struct Code : Xbyak::CodeGenerator {
        Code(bool grow) : Xbyak::CodeGenerator(grow ? 128 : 4096, grow ? Xbyak::AutoGrow : 0) {
            xor_(eax, eax);
            {
                Label label1;
                Label label2;
...
struct GetAddressCode2 : Xbyak::CodeGenerator {
    ...
    explicit GetAddressCode2(int size)
        : Xbyak::CodeGenerator(size, size == 4096 ? 0 : Xbyak::AutoGrow), a1(0), a3(0) {
...
CYBOZU_TEST_AUTO(release_label_after_code) {
    puts("---");
    {
        Label L1, L2, L3, L4, L5;
        {
            ...
            code.assignL(L4, L1);
            L5 = L1;
            printf("id=%d %d %d %d %d\n", L1.getId(), L2.getId(), L3.getId(), L4.getId(),
                   L5.getId());
        }
...
CYBOZU_TEST_AUTO(setDefaultJmpNEAR) {
    ...
    const struct {
        ...
        {false, false, T_SHORT, 2, 2}, {false, false, T_NEAR, 5, 5},
        {false, true, T_SHORT, 0, 0},  {false, true, T_NEAR, 5, 5},

        {true, false, T_SHORT, 2, 2},  {true, false, T_NEAR, 5, 5},
        {true, true, T_SHORT, 0, 0},   {true, true, T_NEAR, 5, 5},

        {false, false, T_AUTO, 2, 5},  {false, true, T_AUTO, 0, 5},
        {true, false, T_AUTO, 2, 2},   {true, true, T_AUTO, 5, 5},
    };
