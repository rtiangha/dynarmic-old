...
void putOpmask(bool only64bit) {
    if (only64bit) {
        puts("void kmovq(const Opmask& k, const Reg64& r) { opVex(k, 0, r, T_L0 | T_0F | T_F2 | "
             "T_W1, 0x92); }");
        puts("void kmovq(const Reg64& r, const Opmask& k) { opVex(r, 0, k, T_L0 | T_0F | T_F2 | "
             "T_W1, 0x93); }");
        return;
    }

    {
        const struct Tbl {
            ...
            {"kadd", 0x4A}, {"kand", 0x41},  {"kandn", 0x42},
            {"kor", 0x45},  {"kxnor", 0x46}, {"kxor", 0x47},
        };
        for (size_t i = 0; i < NUM_OF_ARRAY(tbl); i++) {
            const Tbl& p = tbl[i];
            printf("void %sw(const Opmask& r1, const Opmask& r2, const Opmask& r3) { opVex(r1, "
                   "&r2, r3, T_L1 | T_0F | T_W0, 0x%02X); }\n",
                   p.name, p.code);
            printf("void %sq(const Opmask& r1, const Opmask& r2, const Opmask& r3) { opVex(r1, "
                   "&r2, r3, T_L1 | T_0F | T_W1, 0x%02X); }\n",
                   p.name, p.code);
            printf("void %sb(const Opmask& r1, const Opmask& r2, const Opmask& r3) { opVex(r1, "
                   "&r2, r3, T_L1 | T_0F | T_66 | T_W0, 0x%02X); }\n",
                   p.name, p.code);
            printf("void %sd(const Opmask& r1, const Opmask& r2, const Opmask& r3) { opVex(r1, "
                   "&r2, r3, T_L1 | T_0F | T_66 | T_W1, 0x%02X); }\n",
                   p.name, p.code);
        }
        printf("void kunpckbw(const Opmask& r1, const Opmask& r2, const Opmask& r3) { opVex(r1, "
               "&r2, r3, T_L1 | T_0F | T_66 | T_W0, 0x4B); }\n");
        printf("void kunpckwd(const Opmask& r1, const Opmask& r2, const Opmask& r3) { opVex(r1, "
               "&r2, r3, T_L1 | T_0F | T_W0, 0x4B); }\n");
        printf("void kunpckdq(const Opmask& r1, const Opmask& r2, const Opmask& r3) { opVex(r1, "
               "&r2, r3, T_L1 | T_0F | T_W1, 0x4B); }\n");
    }
    {
        const struct Tbl {
            const char* name;
        ...
        for (size_t i = 0; i < NUM_OF_ARRAY(tbl); i++) {
            const Tbl& p = tbl[i];
            printf("void %sw(const Opmask& r1, const Opmask& r2) { opVex(r1, 0, r2, T_0F | T_W0, "
                   "0x%02X); }\n",
                   p.name, p.code);
            printf("void %sq(const Opmask& r1, const Opmask& r2) { opVex(r1, 0, r2, T_0F | T_W1, "
                   "0x%02X); }\n",
                   p.name, p.code);
            printf("void %sb(const Opmask& r1, const Opmask& r2) { opVex(r1, 0, r2, T_0F | T_66 | "
                   "T_W0, 0x%02X); }\n",
                   p.name, p.code);
            printf("void %sd(const Opmask& r1, const Opmask& r2) { opVex(r1, 0, r2, T_0F | T_66 | "
                   "T_W1, 0x%02X); }\n",
                   p.name, p.code);
        }
    }
    {
        ...
        for (size_t i = 0; i < NUM_OF_ARRAY(tbl); i++) {
            const Tbl& p = tbl[i];
            printf("void %sw(const Opmask& r1, const Opmask& r2, uint8_t imm) { opVex(r1, 0, r2, "
                   "T_66 | T_0F3A | T_W1, 0x%02X, imm); }\n",
                   p.name, p.code);
            printf("void %sq(const Opmask& r1, const Opmask& r2, uint8_t imm) { opVex(r1, 0, r2, "
                   "T_66 | T_0F3A | T_W1, 0x%02X, imm); }\n",
                   p.name, p.code + 1);
            printf("void %sb(const Opmask& r1, const Opmask& r2, uint8_t imm) { opVex(r1, 0, r2, "
                   "T_66 | T_0F3A | T_W0, 0x%02X, imm); }\n",
                   p.name, p.code);
            printf("void %sd(const Opmask& r1, const Opmask& r2, uint8_t imm) { opVex(r1, 0, r2, "
                   "T_66 | T_0F3A | T_W0, 0x%02X, imm); }\n",
                   p.name, p.code + 1);
        }
    }
    puts("void kmovw(const Opmask& k, const Operand& op) { if (!op.isMEM() && !op.isOPMASK()) "
         "XBYAK_THROW(ERR_BAD_COMBINATION) opVex(k, 0, op, T_L0 | T_0F | T_W0, 0x90); }");
    puts("void kmovq(const Opmask& k, const Operand& op) { if (!op.isMEM() && !op.isOPMASK()) "
         "XBYAK_THROW(ERR_BAD_COMBINATION) opVex(k, 0, op, T_L0 | T_0F | T_W1, 0x90); }");
    puts("void kmovb(const Opmask& k, const Operand& op) { if (!op.isMEM() && !op.isOPMASK()) "
         "XBYAK_THROW(ERR_BAD_COMBINATION) opVex(k, 0, op, T_L0 | T_0F | T_66 | T_W0, 0x90); }");
    puts("void kmovd(const Opmask& k, const Operand& op) { if (!op.isMEM() && !op.isOPMASK()) "
         "XBYAK_THROW(ERR_BAD_COMBINATION) opVex(k, 0, op, T_L0 | T_0F | T_66 | T_W1, 0x90); }");

    puts("void kmovw(const Address& addr, const Opmask& k) { opVex(k, 0, addr, T_L0 | T_0F | T_W0, "
         "0x91); }");
    puts("void kmovq(const Address& addr, const Opmask& k) { opVex(k, 0, addr, T_L0 | T_0F | T_W1, "
         "0x91); }");
    puts("void kmovb(const Address& addr, const Opmask& k) { opVex(k, 0, addr, T_L0 | T_0F | T_66 "
         "| T_W0, 0x91); }");
    puts("void kmovd(const Address& addr, const Opmask& k) { opVex(k, 0, addr, T_L0 | T_0F | T_66 "
         "| T_W1, 0x91); }");
    ...
    puts("void kmovw(const Reg32& r, const Opmask& k) { opVex(r, 0, k, T_L0 | T_0F | T_W0, 0x93); "
         "}");
    puts("void kmovb(const Opmask& k, const Reg32& r) { opVex(k, 0, r, T_L0 | T_0F | T_66 | T_W0, "
         "0x92); }");
    puts("void kmovb(const Reg32& r, const Opmask& k) { opVex(r, 0, k, T_L0 | T_0F | T_66 | T_W0, "
         "0x93); }");
    puts("void kmovd(const Opmask& k, const Reg32& r) { opVex(k, 0, r, T_L0 | T_0F | T_F2 | T_W0, "
         "0x92); }");
    puts("void kmovd(const Reg32& r, const Opmask& k) { opVex(r, 0, k, T_L0 | T_0F | T_F2 | T_W0, "
         "0x93); }");
}
...
void putVcmp() {
    ...
    for (size_t i = 0; i < NUM_OF_ARRAY(tbl); i++) {
        ...
        printf("void %s(const Opmask& k, const Xmm& x, const Operand& op%s) { opAVX_K_X_XM(k, x, "
               "op, %s, 0x%02X%s); }\n",
...
void putX_XM() {
    ...
    puts("void vpabsq(const Xmm& x, const Operand& op) { opAVX_X_XM_IMM(x, op, T_66 | T_0F38 | "
         "T_MUST_EVEX | T_EW1 | T_B64 | T_YMM, 0x1F); }");

    puts("void vexp2pd(const Zmm& z, const Operand& op) { opAVX_X_XM_IMM(z, op, T_66 | T_0F38 | "
         "T_MUST_EVEX | T_YMM | T_EW1 | T_B64 | T_SAE_Z, 0xC8); }");
    puts("void vexp2ps(const Zmm& z, const Operand& op) { opAVX_X_XM_IMM(z, op, T_66 | T_0F38 | "
         "T_MUST_EVEX | T_YMM | T_EW0 | T_B32 | T_SAE_Z, 0xC8); }");
    puts("void vrcp28pd(const Zmm& z, const Operand& op) { opAVX_X_XM_IMM(z, op, T_66 | T_0F38 | "
         "T_MUST_EVEX | T_YMM | T_EW1 | T_B64 | T_SAE_Z, 0xCA); }");
    puts("void vrcp28ps(const Zmm& z, const Operand& op) { opAVX_X_XM_IMM(z, op, T_66 | T_0F38 | "
         "T_MUST_EVEX | T_YMM | T_EW0 | T_B32 | T_SAE_Z, 0xCA); }");

    puts("void vrsqrt28pd(const Zmm& z, const Operand& op) { opAVX_X_XM_IMM(z, op, T_66 | T_0F38 | "
         "T_MUST_EVEX | T_YMM | T_EW1 | T_B64 | T_SAE_Z, 0xCC); }");
    puts("void vrsqrt28ps(const Zmm& z, const Operand& op) { opAVX_X_XM_IMM(z, op, T_66 | T_0F38 | "
         "T_MUST_EVEX | T_YMM | T_EW0 | T_B32 | T_SAE_Z, 0xCC); }");
}
...
void putM_X() {
    const struct Tbl {
        ...
        {0x7F, "vmovdqa32",
         T_66 | T_0F | T_MUST_EVEX | T_YMM | T_EW0 | T_ER_X | T_ER_Y | T_ER_Z | T_M_K},
        {0x7F, "vmovdqa64",
         T_66 | T_0F | T_MUST_EVEX | T_YMM | T_EW1 | T_ER_X | T_ER_Y | T_ER_Z | T_M_K},
        {0x7F, "vmovdqu8",
         T_F2 | T_0F | T_MUST_EVEX | T_YMM | T_EW0 | T_ER_X | T_ER_Y | T_ER_Z | T_M_K},
        {0x7F, "vmovdqu16",
         T_F2 | T_0F | T_MUST_EVEX | T_YMM | T_EW1 | T_ER_X | T_ER_Y | T_ER_Z | T_M_K},
        {0x7F, "vmovdqu32",
         T_F3 | T_0F | T_MUST_EVEX | T_YMM | T_EW0 | T_ER_X | T_ER_Y | T_ER_Z | T_M_K},
        {0x7F, "vmovdqu64",
         T_F3 | T_0F | T_MUST_EVEX | T_YMM | T_EW1 | T_ER_X | T_ER_Y | T_ER_Z | T_M_K},
    };
    for (size_t i = 0; i < NUM_OF_ARRAY(tbl); i++) {
        const Tbl* p = &tbl[i];
        std::string type = type2String(p->type);
...
void putX_X_XM_IMM() {
    const struct Tbl {
        ...
        {0x72, "vcvtne2ps2bf16", T_F2 | T_0F38 | T_YMM | T_MUST_EVEX | T_EW0 | T_SAE_Z | T_B32,
         false},
    ...
    for (size_t i = 0; i < NUM_OF_ARRAY(tbl); i++) {
        const Tbl* p = &tbl[i];
        std::string type = type2String(p->type);
        printf("void %s(const Xmm& x1, const Xmm& x2, const Operand& op%s) { opAVX_X_X_XM(x1, x2, "
               "op, %s, 0x%02X%s); }\n",
               p->name, p->hasIMM ? ", uint8_t imm" : "", type.c_str(), p->code,
               p->hasIMM ? ", imm" : "");
    }
...
void putShift() {
    ...
    for (size_t i = 0; i < NUM_OF_ARRAY(tbl); i++) {
        const Tbl& p = tbl[i];
        std::string type = type2String(p.type);
        printf("void %s(const Xmm& x, const Operand& op, uint8_t imm) { "
...
void putExtractInsert() {
    {
        ...
        for (size_t i = 0; i < NUM_OF_ARRAY(tbl); i++) {
            ...
            std::string type = type2String(p.type);
            const char* kind =
                p.isZMM ? "Operand::MEM | Operand::YMM" : "Operand::MEM | Operand::XMM";
            printf("void %s(const Operand& op, const %s& r, uint8_t imm) { if (!op.is(%s)) "
                   "XBYAK_THROW(ERR_BAD_COMBINATION) opVex(r, 0, op, %s, 0x%2X, imm); }\n",
    ...
    }
    {
        ...
        for (size_t i = 0; i < NUM_OF_ARRAY(tbl); i++) {
            ...
            const char* cond =
                p.isZMM ? "op.is(Operand::MEM | Operand::YMM)"
...
void putBroadcast(bool only64bit) {
    ...
    if (only64bit)
        return;
    puts("void vbroadcastf32x2(const Ymm& y, const Operand& op) { opAVX_X_XM_IMM(y, op, T_66 | "
         "T_0F38 | T_YMM | T_MUST_EVEX | T_EW0 | T_N8, 0x19); }");
    puts("void vbroadcastf32x4(const Ymm& y, const Address& addr) { opAVX_X_XM_IMM(y, addr, T_66 | "
         "T_0F38 | T_YMM | T_MUST_EVEX | T_EW0 | T_N16, 0x1A); }");
    puts("void vbroadcastf64x2(const Ymm& y, const Address& addr) { opAVX_X_XM_IMM(y, addr, T_66 | "
         "T_0F38 | T_YMM | T_MUST_EVEX | T_EW1 | T_N16, 0x1A); }");
    puts("void vbroadcastf64x4(const Zmm& y, const Address& addr) { opAVX_X_XM_IMM(y, addr, T_66 | "
         "T_0F38 | T_YMM | T_MUST_EVEX | T_EW1 | T_N32, 0x1B); }");
    puts("void vbroadcastf32x8(const Zmm& y, const Address& addr) { opAVX_X_XM_IMM(y, addr, T_66 | "
         "T_0F38 | T_YMM | T_MUST_EVEX | T_EW0 | T_N32, 0x1B); }");

    puts("void vbroadcasti32x2(const Xmm& x, const Operand& op) { opAVX_X_XM_IMM(x, op, T_66 | "
         "T_0F38 | T_YMM | T_MUST_EVEX | T_EW0 | T_N8, 0x59); }");
    puts("void vbroadcasti32x4(const Ymm& y, const Operand& op) { opAVX_X_XM_IMM(y, op, T_66 | "
         "T_0F38 | T_YMM | T_MUST_EVEX | T_EW0 | T_N16, 0x5A); }");
    puts("void vbroadcasti64x2(const Ymm& y, const Operand& op) { opAVX_X_XM_IMM(y, op, T_66 | "
         "T_0F38 | T_YMM | T_MUST_EVEX | T_EW1 | T_N16, 0x5A); }");
    puts("void vbroadcasti32x8(const Zmm& z, const Operand& op) { opAVX_X_XM_IMM(z, op, T_66 | "
         "T_0F38 | T_YMM | T_MUST_EVEX | T_EW0 | T_N32, 0x5B); }");
    puts("void vbroadcasti64x4(const Zmm& z, const Operand& op) { opAVX_X_XM_IMM(z, op, T_66 | "
         "T_0F38 | T_YMM | T_MUST_EVEX | T_EW1 | T_N32, 0x5B); }");
}

void putCvt() {
    puts("void vcvtpd2udq(const Xmm& x, const Operand& op) { opCvt2(x, op, T_0F | T_YMM | "
         "T_MUST_EVEX | T_EW1 | T_B64 | T_ER_Z, 0x79); }");
    puts("void vcvtps2qq(const Xmm& x, const Operand& op) { checkCvt1(x, op); opVex(x, 0, op, T_66 "
         "| T_0F | T_YMM | T_MUST_EVEX | T_EW0 | T_B32 | T_N8 | T_N_VL | T_ER_Y, 0x7B); }");
    puts("void vcvtps2uqq(const Xmm& x, const Operand& op) { checkCvt1(x, op); opVex(x, 0, op, "
         "T_66 | T_0F | T_YMM | T_MUST_EVEX | T_EW0 | T_B32 | T_N8 | T_N_VL | T_ER_Y, 0x79); }");
    puts("void vcvtqq2ps(const Xmm& x, const Operand& op) { opCvt2(x, op, T_0F | T_YMM | "
         "T_MUST_EVEX | T_EW1 | T_B64 | T_ER_Z, 0x5B); }");
    puts("void vcvttpd2udq(const Xmm& x, const Operand& op) { opCvt2(x, op, T_0F | T_YMM | "
         "T_MUST_EVEX | T_EW1 | T_B64 | T_SAE_Z, 0x78); }");
    puts("void vcvttps2qq(const Xmm& x, const Operand& op) { checkCvt1(x, op); opVex(x, 0, op, "
         "T_66 | T_0F | T_YMM | T_MUST_EVEX | T_EW0 | T_B32 | T_N8 | T_N_VL | T_SAE_Y, 0x7A); }");
    puts("void vcvttps2uqq(const Xmm& x, const Operand& op) { checkCvt1(x, op); opVex(x, 0, op, "
         "T_66 | T_0F | T_YMM | T_MUST_EVEX | T_EW0 | T_B32 | T_N8 | T_N_VL | T_SAE_Y, 0x78); }");
    puts("void vcvtudq2pd(const Xmm& x, const Operand& op) { checkCvt1(x, op); opVex(x, 0, op, "
         "T_F3 | T_0F | T_YMM | T_MUST_EVEX | T_EW0 | T_B32 | T_N8 | T_N_VL, 0x7A); }");

    puts("void vcvtsd2usi(const Reg32e& r, const Operand& op) { int type = (T_F2 | T_0F | "
         "T_MUST_EVEX | T_N8 | T_ER_X) | (r.isREG(64) ? T_EW1 : T_EW0); "
         "opAVX_X_X_XM(Xmm(r.getIdx()), xm0, op, type, 0x79); }");
    puts("void vcvtss2usi(const Reg32e& r, const Operand& op) { int type = (T_F3 | T_0F | "
         "T_MUST_EVEX | T_N4 | T_ER_X) | (r.isREG(64) ? T_EW1 : T_EW0); "
         "opAVX_X_X_XM(Xmm(r.getIdx()), xm0, op, type, 0x79); }");
    puts("void vcvttsd2usi(const Reg32e& r, const Operand& op) { int type = (T_F2 | T_0F | "
         "T_MUST_EVEX | T_N8 | T_SAE_X) | (r.isREG(64) ? T_EW1 : T_EW0); "
         "opAVX_X_X_XM(Xmm(r.getIdx()), xm0, op, type, 0x78); }");
    puts("void vcvttss2usi(const Reg32e& r, const Operand& op) { int type = (T_F3 | T_0F | "
         "T_MUST_EVEX | T_N4 | T_SAE_X) | (r.isREG(64) ? T_EW1 : T_EW0); "
         "opAVX_X_X_XM(Xmm(r.getIdx()), xm0, op, type, 0x78); }");
    puts("void vcvtuqq2ps(const Xmm& x, const Operand& op) { opCvt2(x, op, T_F2 | T_0F | T_YMM | "
         "T_MUST_EVEX | T_EW1 | T_B64 | T_ER_Z, 0x7A); }");
    puts("void vcvtusi2sd(const Xmm& x1, const Xmm& x2, const Operand& op) { opCvt3(x1, x2, op, "
         "T_F2 | T_0F | T_MUST_EVEX, T_W1 | T_EW1 | T_ER_X | T_N8, T_W0 | T_EW0 | T_N4, 0x7B); }");
    puts("void vcvtusi2ss(const Xmm& x1, const Xmm& x2, const Operand& op) { opCvt3(x1, x2, op, "
         "T_F3 | T_0F | T_MUST_EVEX | T_ER_X, T_W1 | T_EW1 | T_N8, T_W0 | T_EW0 | T_N4, 0x7B); }");
}
...
void putShuff() {
    puts("void vshuff32x4(const Ymm& y1, const Ymm& y2, const Operand& op, uint8_t imm) { "
         "opAVX_X_X_XM(y1, y2, op, T_66 | T_0F3A | T_YMM | T_MUST_EVEX | T_EW0 | T_B32, 0x23, "
         "imm); }");
    puts("void vshuff64x2(const Ymm& y1, const Ymm& y2, const Operand& op, uint8_t imm) { "
         "opAVX_X_X_XM(y1, y2, op, T_66 | T_0F3A | T_YMM | T_MUST_EVEX | T_EW1 | T_B64, 0x23, "
         "imm); }");
    puts("void vshufi32x4(const Ymm& y1, const Ymm& y2, const Operand& op, uint8_t imm) { "
         "opAVX_X_X_XM(y1, y2, op, T_66 | T_0F3A | T_YMM | T_MUST_EVEX | T_EW0 | T_B32, 0x43, "
         "imm); }");
    puts("void vshufi64x2(const Ymm& y1, const Ymm& y2, const Operand& op, uint8_t imm) { "
         "opAVX_X_X_XM(y1, y2, op, T_66 | T_0F3A | T_YMM | T_MUST_EVEX | T_EW1 | T_B64, 0x43, "
         "imm); }");
}

void putMov() {
    puts("void vpmovm2b(const Xmm& x, const Opmask& k) { opVex(x, 0, k, T_F3 | T_0F38 | "
         "T_MUST_EVEX | T_YMM | T_EW0, 0x28); }");
    puts("void vpmovm2w(const Xmm& x, const Opmask& k) { opVex(x, 0, k, T_F3 | T_0F38 | "
         "T_MUST_EVEX | T_YMM | T_EW1, 0x28); }");
    puts("void vpmovm2d(const Xmm& x, const Opmask& k) { opVex(x, 0, k, T_F3 | T_0F38 | "
         "T_MUST_EVEX | T_YMM | T_EW0, 0x38); }");
    puts("void vpmovm2q(const Xmm& x, const Opmask& k) { opVex(x, 0, k, T_F3 | T_0F38 | "
         "T_MUST_EVEX | T_YMM | T_EW1, 0x38); }");

    puts("void vpmovb2m(const Opmask& k, const Xmm& x) { opVex(k, 0, x, T_F3 | T_0F38 | "
         "T_MUST_EVEX | T_YMM | T_EW0, 0x29); }");
    puts("void vpmovw2m(const Opmask& k, const Xmm& x) { opVex(k, 0, x, T_F3 | T_0F38 | "
         "T_MUST_EVEX | T_YMM | T_EW1, 0x29); }");
    puts("void vpmovd2m(const Opmask& k, const Xmm& x) { opVex(k, 0, x, T_F3 | T_0F38 | "
         "T_MUST_EVEX | T_YMM | T_EW0, 0x39); }");
    puts("void vpmovq2m(const Opmask& k, const Xmm& x) { opVex(k, 0, x, T_F3 | T_0F38 | "
         "T_MUST_EVEX | T_YMM | T_EW1, 0x39); }");
...
void putMisc() {
    puts("void vpbroadcastmb2q(const Xmm& x, const Opmask& k) { opVex(x, 0, k, T_F3 | T_0F38 | "
         "T_YMM | T_MUST_EVEX | T_EW1, 0x2A); }");
    puts("void vpbroadcastmw2d(const Xmm& x, const Opmask& k) { opVex(x, 0, k, T_F3 | T_0F38 | "
         "T_YMM | T_MUST_EVEX | T_EW0, 0x3A); }");
    {
        const struct Tbl {
            const char* name;
            int zm;
        ...
        for (size_t i = 0; i < NUM_OF_ARRAY(tbl); i++) {
            ...
            printf("void %s(const Address& addr) { opGatherFetch(addr, zm%d, %s, 0x%2X, "
                   "Operand::%s); }\n",
                   p.name, p.zm, type.c_str(), p.code, p.isZmm ? "ZMM" : "YMM");
        }
    }

    puts("void vfpclasspd(const Opmask& k, const Operand& op, uint8_t imm) { if "
         "(!op.isBit(128|256|512)) XBYAK_THROW(ERR_BAD_MEM_SIZE) opVex(k.changeBit(op.getBit()), "
         "0, op, T_66 | T_0F3A | T_MUST_EVEX | T_YMM | T_EW1 | T_B64, 0x66, imm); }");
    puts("void vfpclassps(const Opmask& k, const Operand& op, uint8_t imm) { if "
         "(!op.isBit(128|256|512)) XBYAK_THROW(ERR_BAD_MEM_SIZE) opVex(k.changeBit(op.getBit()), "
         "0, op, T_66 | T_0F3A | T_MUST_EVEX | T_YMM | T_EW0 | T_B32, 0x66, imm); }");
    puts("void vfpclasssd(const Opmask& k, const Operand& op, uint8_t imm) { if (!op.isXMEM()) "
         "XBYAK_THROW(ERR_BAD_MEM_SIZE) opVex(k, 0, op, T_66 | T_0F3A | T_MUST_EVEX | T_EW1 | "
         "T_N8, 0x67, imm); }");
    puts("void vfpclassss(const Opmask& k, const Operand& op, uint8_t imm) { if (!op.isXMEM()) "
         "XBYAK_THROW(ERR_BAD_MEM_SIZE) opVex(k, 0, op, T_66 | T_0F3A | T_MUST_EVEX | T_EW0 | "
         "T_N4, 0x67, imm); }");

    puts("void vpshufbitqmb(const Opmask& k, const Xmm& x, const Operand& op) { opVex(k, &x, op, "
         "T_66 | T_0F38 | T_EW0 | T_YMM | T_MUST_EVEX, 0x8F); }");
    puts("void vcvtneps2bf16(const Xmm& x, const Operand& op) { opCvt2(x, op, T_F3 | T_0F38 | "
         "T_EW0 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B32, 0x72); }");

    puts("void vp2intersectd(const Opmask& k, const Xmm& x, const Operand& op) { if "
         "(k.getOpmaskIdx() != 0) XBYAK_THROW(ERR_OPMASK_IS_ALREADY_SET) opAVX_K_X_XM(k, x, op, "
         "T_F2 | T_0F38 | T_YMM | T_EVEX | T_EW0 | T_B32, 0x68); }");
    puts("void vp2intersectq(const Opmask& k, const Xmm& x, const Operand& op) { if "
         "(k.getOpmaskIdx() != 0) XBYAK_THROW(ERR_OPMASK_IS_ALREADY_SET) opAVX_K_X_XM(k, x, op, "
         "T_F2 | T_0F38 | T_YMM | T_EVEX | T_EW1 | T_B64, 0x68); }");
}

void putV4FMA() {
    puts("void v4fmaddps(const Zmm& z1, const Zmm& z2, const Address& addr) { opAVX_X_X_XM(z1, z2, "
         "addr, T_0F38 | T_F2 | T_EW0 | T_YMM | T_MUST_EVEX | T_N16, 0x9A); }");
    puts("void v4fnmaddps(const Zmm& z1, const Zmm& z2, const Address& addr) { opAVX_X_X_XM(z1, "
         "z2, addr, T_0F38 | T_F2 | T_EW0 | T_YMM | T_MUST_EVEX | T_N16, 0xAA); }");
    puts("void v4fmaddss(const Xmm& x1, const Xmm& x2, const Address& addr) { opAVX_X_X_XM(x1, x2, "
         "addr, T_0F38 | T_F2 | T_EW0 | T_MUST_EVEX | T_N16, 0x9B); }");
    puts("void v4fnmaddss(const Xmm& x1, const Xmm& x2, const Address& addr) { opAVX_X_X_XM(x1, "
         "x2, addr, T_0F38 | T_F2 | T_EW0 | T_MUST_EVEX | T_N16, 0xAB); }");
    puts("void vp4dpwssd(const Zmm& z1, const Zmm& z2, const Address& addr) { opAVX_X_X_XM(z1, z2, "
         "addr, T_0F38 | T_F2 | T_EW0 | T_YMM | T_MUST_EVEX | T_N16, 0x52); }");
    puts("void vp4dpwssds(const Zmm& z1, const Zmm& z2, const Address& addr) { opAVX_X_X_XM(z1, "
         "z2, addr, T_0F38 | T_F2 | T_EW0 | T_YMM | T_MUST_EVEX | T_N16, 0x53); }");
}
