...
public:
    ...
    */
    FuncGen(const std::vector<std::string>& varTbl)
        : constTblPos_(0), regIdx_(-1)
#ifdef XBYAK32
          ,
          valTbl_(eax), tbl_(edx)
#elif defined(XBYAK64_WIN)
          ,
          valTbl_(rcx), tbl_(rdx)
#else
          ,
          valTbl_(rdi), tbl_(rsi)
#endif
    {
#ifdef XBYAK32
    ...
    void genVal(const char* begin, const char* end) {
        ...
        if (varMap_.find(var) == varMap_.end())
...
struct Grammar : public boost::spirit::classic::grammar<Grammar> {
    ...
    struct definition {
        ...
        definition(const Grammar& self) {
            ...
            poly2 = real_p[bind(&FuncGen::genPush, ref(self.f_), _1)] | var | '(' >> poly0 >> ')';
        }
...
#ifdef XBYAK32
        puts("32bit mode");
        void (*func)(double* ret, const double* valTbl) =
            funcGen.getCode<void (*)(double*, const double*)>();
#else
        puts("64bit mode");
