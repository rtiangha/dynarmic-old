           ...
           */
            ...
            */
            ...
            numCores_[CoreLevel - 1] =
                (std::max)(numCores_[SmtLevel - 1], numCores_[CoreLevel - 1]);
        } else {
            /*
...
public:
    ...
    */
    ...
    Cpu()
        : type_(NONE), x2APIC_supported_(false), numCores_(), dataCacheSize_(),
        ...
        if (type_ & tOSXSAVE) {
            ...
            if ((bv & 6) == 6) {
                ...
                if (ECX & (1U << 12))
                    type_ |= tFMA;
                    // do *not* check AVX-512 state on macOS because it has on-demand AVX-512
                    // support
#if !defined(__APPLE__)
                if (((bv >> 5) & 7) == 7)
#endif
...
public:
    ...
    /*
        ...
        @param tNum [in] num of temporary register(0 <= tNum, with UseRCX, UseRDX) #{pNum + tNum
       [+rcx] + [rdx]} <= 14
    ...
    */
    StackFrame(Xbyak::CodeGenerator* code, int pNum, int tNum = 0, int stackSizeByte = 0,
               bool makeEpilog = true)
        : code_(code), pNum_(pNum), tNum_(tNum & ~(UseRCX | UseRDX)), useRcx_((tNum & UseRCX) != 0),
          useRdx_((tNum & UseRDX) != 0), saveNum_(0), P_(0), makeEpilog_(makeEpilog), p(p_), t(t_) {
...
#ifdef XBYAK64_WIN
            Operand::RCX, Operand::RDX, Operand::R8,  Operand::R9,  Operand::R10, Operand::R11,
