...
void EmitX64::EmitLogicalShiftLeft32(EmitContext& ctx, IR::Inst* inst) {
    ...
    if (!carry_inst) {
        ...
        if (shift_arg.IsImmediate()) {
            ...
            // if (Rs & 0xFF < 32) {
            code.bt(carry.cvt32(),
                    0); // Set the carry flag for correct behaviour in the case when Rs & 0xFF == 0
            code.shl(result, code.cl);
            code.setc(carry.cvt8());
            code.jmp(".end");
            // } else if (Rs & 0xFF > 32) {
...
void EmitX64::EmitLogicalShiftLeft64(EmitContext& ctx, IR::Inst* inst) {
    ...
    if (shift_arg.IsImmediate()) {
        ...
        // ARM differs from the behaviour: It does not mask the count, so shifts above 31 result in
        // zeros.
