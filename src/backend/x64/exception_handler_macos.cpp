...
kern_return_t MachHandler::HandleRequest(x86_thread_state64_t* ts) {
    ...
    if (iter == code_block_infos.end()) {
        fmt::print(stderr,
                   "dynarmic: macOS MachHandler: Exception was not in registered code blocks (rip "
                   "{:#016x})\n",
                   ts->__rip);
        return KERN_FAILURE;
    }
...
mig_external kern_return_t catch_mach_exception_raise_state(
    ...
    if (*flavor != x86_THREAD_STATE64 || old_stateCnt != x86_THREAD_STATE64_COUNT ||
    ...
    std::memcpy(ts, reinterpret_cast<const x86_thread_state64_t*>(old_state),
