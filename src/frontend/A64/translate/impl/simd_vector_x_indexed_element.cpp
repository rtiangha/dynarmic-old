...
bool MultiplyByElement(TranslatorVisitor& v, bool Q, Imm<2> size, Imm<1> L, Imm<1> M, Imm<4> Vmlo,
                       Imm<1> H, Vec Vn, Vec Vd, ExtraBehavior extra_behavior) {
    ...
    const IR::U128 operand2 =
        v.ir.VectorBroadcast(esize, v.ir.VectorGetElement(esize, v.V(idxdsize, Vm), index));
...
bool FPMultiplyByElement(TranslatorVisitor& v, bool Q, bool sz, Imm<1> L, Imm<1> M, Imm<4> Vmlo,
    ...
    const IR::U128 operand1 = v.V(datasize, Vn);
    const IR::U128 operand2 =
        Q ? v.ir.VectorBroadcast(esize, element2) : v.ir.VectorBroadcastLower(esize, element2);
    const IR::U128 operand3 = v.V(datasize, Vd);
...
bool MultiplyLong(TranslatorVisitor& v, bool Q, Imm<2> size, Imm<1> L, Imm<1> M, Imm<4> Vmlo,
    ...
    const auto extend_operands = [&](const IR::U128& lhs, const IR::U128& rhs) {
        ...
        return std::make_pair(v.ir.VectorZeroExtend(esize, lhs), v.ir.VectorZeroExtend(esize, rhs));
    };

    const IR::U128 operand1 = v.Vpart(datasize, Vn, Q);
    const IR::U128 operand2 = v.V(idxsize, Vm);
    const IR::U128 index_vector =
        v.ir.VectorBroadcast(esize, v.ir.VectorGetElement(esize, operand2, index));
...
bool TranslatorVisitor::SMLAL_elt(bool Q, Imm<2> size, Imm<1> L, Imm<1> M, Imm<4> Vmlo, Imm<1> H,
                                  Vec Vn, Vec Vd) {
    return MultiplyLong(*this, Q, size, L, M, Vmlo, H, Vn, Vd, ExtraBehavior::Accumulate,
                        Signedness::Signed);
}

bool TranslatorVisitor::SMLSL_elt(bool Q, Imm<2> size, Imm<1> L, Imm<1> M, Imm<4> Vmlo, Imm<1> H,
                                  Vec Vn, Vec Vd) {
    return MultiplyLong(*this, Q, size, L, M, Vmlo, H, Vn, Vd, ExtraBehavior::Subtract,
                        Signedness::Signed);
}

bool TranslatorVisitor::SMULL_elt(bool Q, Imm<2> size, Imm<1> L, Imm<1> M, Imm<4> Vmlo, Imm<1> H,
                                  Vec Vn, Vec Vd) {
    return MultiplyLong(*this, Q, size, L, M, Vmlo, H, Vn, Vd, ExtraBehavior::None,
                        Signedness::Signed);
}
...
bool TranslatorVisitor::SQDMULL_elt_2(bool Q, Imm<2> size, Imm<1> L, Imm<1> M, Imm<4> Vmlo,
    ...
    const IR::U128 index_vector =
        ir.VectorBroadcast(esize, ir.VectorGetElement(esize, operand2, index));
    const IR::U128 result =
...
bool TranslatorVisitor::SQDMULH_elt_2(bool Q, Imm<2> size, Imm<1> L, Imm<1> M, Imm<4> Vmlo,
    ...
    const IR::U128 index_vector =
        ir.VectorBroadcast(esize, ir.VectorGetElement(esize, operand2, index));
    const IR::U128 result =
        ir.VectorSignedSaturatedDoublingMultiply(esize, operand1, index_vector).upper;
...
bool TranslatorVisitor::SQRDMULH_elt_2(bool Q, Imm<2> size, Imm<1> L, Imm<1> M, Imm<4> Vmlo,
    ...
    const IR::UpperAndLower multiply =
        ir.VectorSignedSaturatedDoublingMultiply(esize, operand1, index_vector);
    const IR::U128 result =
